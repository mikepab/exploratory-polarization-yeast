function extract_data_cv(filename)
load(filename);
write_to_file=true;
units='nM';
...
if write_to_file
    for i=1:7
        currmat = squeeze(std(localPherConc_em(i,:),[],2)) ./ squeeze(mean(localPherConc_em(i,:),2))*100;
        writematrix(currmat,sprintf('%s_em_angle_%i_cv.csv',fname_noext,i));

        currmat = squeeze(std(localPherConc_rc(i,:),[],2)) ./ squeeze(mean(localPherConc_rc(i,:),2))*100;
        writematrix(currmat,sprintf('%s_rc_angle_%i_cv.csv',fname_noext,i));
    end
end
