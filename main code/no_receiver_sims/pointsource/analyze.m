...
[nPerShell,thresholds] = count_per_shell(pos_em,d_sphere,sep_dist,0.25); 

[~,savename,~] = fileparts(target_file); % get name separate from extension

save(sprintf('%s_processed',savename),'nPerShell');
end
...
for i=1:numel(pos.pheromone)
    ...
    if ~isempty(pos.pheromone{i})
        ...
        if size(tmp,1)==1
            pos_em.pheromone{i} = tmp;%tmp(norm(tmp)<threshold_dist,:);
        elseif size(tmp,1)>1
            pos_em.pheromone{i} = tmp;%tmp(vecnorm(tmp,2,2)<threshold_dist,:);
        end
        ...
        tmp = pos.pheromone{i} - [d_sphere/2+sep_dist/2,0,0];    
        if size(tmp,1)==1
            pos_rc.pheromone{i} = tmp;%tmp(norm(tmp)<threshold_dist,:);
        elseif size(tmp,1)>1
            pos_rc.pheromone{i} = tmp;%tmp(vecnorm(tmp,2,2)<threshold_dist,:);
        end
...
for i=1:nframes
    ...
    if ~isempty(pos.pheromone{i})
        ...
        for j=2:numel(thresholds)
            nPerShell(i,j-1) = sum( (distances > thresholds(j-1)) .* (distances < thresholds(j)) );
        end
