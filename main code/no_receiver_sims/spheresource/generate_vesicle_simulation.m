...
warning('Assuming that the diameter of the sphere is 5 micron')
warning('Assuming that the separation distance of the sphere is 0.25 micron')
write_vesicle_events(fid,5/2,0.25);
...
function write_vesicle_events(fid,radius,sep_dist)
% Generate timepoints at which to create vesicles.
t=0;
times=[];
while t<(15) % 11 second simulation time
    vesicle_time = exprnd(1.188); % new vesicle every 1.188 seconds on average
    t=t+vesicle_time;
    times=[times;t];
end

for i=1:numel(times)
    % Release 660 pheromone once when simulation time exceeds times(i)
    % Hard-coded the emitted center as.
    [x,y,z] = draw_random_point_on_sphere(radius+0.001,-(radius+sep_dist/2),0,0,1);
    fprintf(fid,'cmd @ %g pointsource pheromone 1663 %g %g %g\n',times(i),x,y,z);
end
end

function [x,y,z] = draw_random_point_on_sphere(radius,x0,y0,z0,npts)
TH = 2*pi*rand(1,npts);
PH = asin(-1+2*rand(1,npts));
[x,y,z] = sph2cart(TH,PH,radius);
x=x+x0;
y=y+y0;
z=z+z0;
end